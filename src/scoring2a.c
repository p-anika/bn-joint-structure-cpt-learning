...
/* MODIFIED BY: cassiopc@gmail.com 22/sept/2014 */
...
#define masseps 1.0e-10
static double newcalc(double aq, double aqr, ARITY aritychild, COUNT *valcount, double *x) {
	double skore = 0., lgamma_aqr;
	int child_val;
	double w = alpha;
	switch(scoretype) {
		case DO_BDEU:
		...
		case DO_MINALPHA:
		{
			int minc=-1,nnz=0,maxc=0,zeroc=-1;
			double f;
			double partial=0.;
			
			for( child_val=0; child_val<aritychild; child_val++) {
				// count the number of non zero counts
				if(valcount[child_val] > 0) {
					nnz++;
					// Find the state with minimum count
					if(minc < 0 || valcount[child_val] < valcount[minc]) minc=child_val;
					// Find the state with maximum count
					if(valcount[child_val] > valcount[maxc]) maxc=child_val;
				} else zeroc=child_val;
				// Define initial Guess
				x[child_val] = aqr;
			}
			if(zeroc >= 0) minc=zeroc;
			DEBUG(fprintf(stderr,"[debug] minc=%d aq=%lf aqr=%lf epsilon=%lf alpha=%lf as",minc,aq,aqr,epsilon,alpha));
			for ( child_val = 0; child_val < aritychild; ++child_val) {
				if(child_val == minc) 
					f = aq - epsilon*(aq-aqr);
				else 
					f = epsilon*aqr;
				DEBUG(fprintf(stderr," %lf(count=%d,f=%lf)",f + valcount[child_val],valcount[child_val],f));
				partial += lgamma(f + valcount[child_val]) - lgamma(f);
			}
		...
		case DO_BIC:
			w *= logbic(nrows)/2.0;
		case DO_AIC:
			skore -= w * (aritychild - 1);
			w = 0.;
			for ( child_val = 0; child_val < aritychild; ++child_val) {
				w += valcount[child_val];
			}
			for ( child_val = 0; child_val < aritychild; ++child_val) {
				if(valcount[child_val] > 0)
					skore += valcount[child_val] * logbic(valcount[child_val]/w);
			}
			break;
		default: fprintf(stderr,"UNDEFINED SCORE CALLED!\n"); exit(1);
	}
	return skore;
}
	...
	)
{
	...
	// Get memory
	double *x = (double *) malloc(sizeof(double) * aritychild);
	...
	for ( cell = 0; cell < incontab_ncells; ++cell )
	{
		...
		for ( val = 0; val < arityvar; ++val )
		  //			if ( valcount[val] > 0 )
			{
				if(scoretype != DO_AIC && scoretype != DO_BIC)
				...
				CASSIO: HERE WE NEED TO CALL THE OPTIMIZER INSTEAD OF USING THE UNIFORM ALPHAS IF NOT BDEU*/
				skore += newcalc(aq,aqr,aritychild,valcount2,x);
	...
	)
{
	...
	*/
	...
	// Get memory
	x = (double *) malloc(sizeof(double) * aritychild);
	...
	for ( cell = 0; cell < contab->ncells; ++cell )
	{
		...
		/* CASSIO: HERE WE NEED TO CALL THE OPTIMIZER INSTEAD OF USING THE UNIFORM ALPHAS*/
		skore += newcalc(aq,aqr,aritychild,valcount,x);
	}
	free(x);
	...
	)
{
	...
	int npa;
	int old_layer_size, new_layer_size;
	CONTAB **old_layer_pa;
	CONTAB **new_layer_pa;
	...
	if ( argc < 4 || argc > 7 )
	{
		fprintf(stderr,"Usage: scoring datafile alpha parentslimit [noprune|prune] [epsilon] [bdeu|min|bic|aic]\n");
		return 1;
	}
	...
	alpha = atof(argv[2]);
	palim = atoi(argv[3]);
	if (argc >= 5 && strcmp(argv[4],"noprune")==0)
		pruning = FALSE;
	else
		pruning = TRUE;
	if (argc >= 6) epsilon = 1.0 - atof(argv[5]);
	else epsilon = 1.0;

	scoretype = DO_BDEU;
	if (argc >= 7) {
		if(strcmp(argv[6],"min")==0)
			scoretype = DO_MINALPHA;
		if(strcmp(argv[6],"bic")==0)
			scoretype = DO_BIC;
		if(strcmp(argv[6],"aic")==0)
			scoretype = DO_AIC;
	}

	fprintf(stderr,"[info] maxparents=%d alpha=%lf epsilon=%lf for ",palim,alpha,epsilon);
	switch(scoretype) {
		case DO_MINALPHA: fprintf(stderr,"min-BDeu\n"); break;
		case DO_AIC: fprintf(stderr,"aic\n"); break;
		case DO_BIC: fprintf(stderr,"bic\n"); break;
		case DO_BDEU: fprintf(stderr,"BDeu\n"); break;
	}

	if(epsilon > 1 || epsilon < 0.0001) { fprintf(stderr,"[error] Epsilon out of range\n"); return 1; }
	...
	if ( ! status )
	{
		fprintf(stderr,"Could not read number of variables \n");
		return 1;
	}
	testchar=fgetc(file);
	if ( testchar != '\n')
	{
		fprintf(stderr,"Number of variables was followed by '%c', not a new line\n", testchar);
		return 1;
	}
	...
	for ( var = 0; var < nvars; ++var)
	{
		...
		if ( ! status )
		{
			fprintf(stderr,"Could not read arity of variable %u\n", var);
			return 1;
		}
	...
	if ( testchar != '\n')
	{
		fprintf(stderr,"Variable arities were followed by '%c', not a new line\n", testchar);
		return 1;
	}
	...
	if ( ! status )
	{
		fprintf(stderr,"Could not read number of data rows \n");
		return 1;
	}
	testchar=fgetc(file);
	if ( testchar != '\n')
	{
		fprintf(stderr,"Number of data rows was followed by '%c', not a new line\n", testchar);
		return 1;
	}
	...
	fprintf(stderr,"[info] data has %d rows and %d variables\n", nrows, nvars);
	...
	for ( row = 0; row < nrows; ++row)
	{
		...
		for ( var = 0; var < nvars; ++var)
		{
			...
			if ( status == EOF )
			{
				fprintf(stderr,"Found fewer rows than %d.\n", nrows);
				return 1;
			}
			if ( ! status )
			{
				fprintf(stderr,"Could not read value of variable %d on row %d\n", var, row);
				return 1;
			}
			if ( ((double) val) - OFFSET < 0 || val - OFFSET >= arity[var] )
			{
				fprintf(stderr,"Invalid value %d for variable %d on row %d (should be non-negative and less than %d) \n", data[var][row], var, row, arity[var]);
				return 1;
			}
		...
		if ( testchar != '\n')
		{
			fprintf(stderr,"Values on row %d (line %d+4) were followed by '%c', not a new line\n", row, row, testchar);
			return 1;
		}
	}
				...
				*/
				...
				{ 
					...
					*/
					...
					if ( pruning )
					{
						double penalty = arity[child] - 1.;
						for ( k = 0; k < nvars; ++k )
							a_parent[k] = 0;
						for ( k = 0; k < npa; ++k ) {
							a_parent[new_vars_ptr[k]] = 1;
							penalty *= arity[new_vars_ptr[k]];
						}
						...
						for ( i = 0; i < n_kept; ++i)
						{
							...
							if ( subset )
							{
								...
								if (scoretype == DO_BDEU && (!last_layer) && (sps_ptr->score > -log(arity[child]) * new_contab_pa_ptr->ncells) ) 
									prune = TRUE; 
								if (scoretype == DO_BIC && (!last_layer) && (sps_ptr->score > -alpha * logbic(nrows) * penalty/2.0) )
									prune = TRUE;
								if (scoretype == DO_AIC && (!last_layer) && (sps_ptr->score > -alpha * penalty) )
									prune = TRUE;
								break;
							}
						}
